...
\twocolumn[
  ...
  \noindent
  The computational validation framework spans five orthogonal methodologies: \textbf{(i)} Ordinary Differential Equation (ODE) systems modeling of the gene expression dynamics across all three modules over 96 hours; \textbf{(ii)} comparative Michaelis-Menten modeling of a NemA$^{*2+}$ catalytic variant whose kinetic parameters ($K_m^* = 16\ \mu$M, $k_{cat}^* = 37.2\ \text{h}^{-1}$) are \textbf{scenario-analysis assumptions} extrapolated from OYE-family active-site mutagenesis precedent \cite{mowafy2010}, not derived from a structural modeling pipeline executed in this study; \textbf{(iii)} a structural evaluation pipeline (PDB 8BPQ, 2.30 \AA\ X-ray) that uses a dimensionless contact-disruption score to rank candidate second-shell mutations in the FMN pocket and a scipy-based geometric placement of CrO$_4^{2-}$ to evaluate substrate positioning relative to FMN --- this structural pipeline is a \textit{consistency check} on the NemA$^{*2+}$ engineering hypothesis, not a kinetic prediction; \textbf{(iv)} a ribosome-allocation genome-scale metabolic model demonstrating that the circuit's $\sim$4.5\% proteome burden is metabolically sustainable at environmentally relevant chromium concentrations; and \textbf{(v)} Luria-Delbr\"uck fluctuation analysis showing that, under the assumed input parameters, the layered containment architecture (genetic kill switch + double auxotrophy $\Delta$thyA $\Delta$dapA) yields a 30-day probability of a viable escapee in a 1,000-liter closed system of $P_{\text{escape}} \approx 6 \times 10^{-17}$, below an acceptable-risk threshold of $10^{-15}$. A one-factor-at-a-time sensitivity sweep (Pillar 4b) shows this headline figure is robust to a 10$\times$ error in the assumed per-bp mutation rate and to a 10$\times$ scale-up of the deployment reactor, but is not robust to a 100$\times$ error in $\mu_{bp}$, at which point $P_{\text{escape}}$ rises to $6 \times 10^{-15}$, above the threshold. A global Latin Hypercube Sampling analysis (Pillar 4c, N = 10,000) over 8 input parameters drawn from literature-bounded ranges finds a median $P_{\text{escape}} = 1.7 \times 10^{-17}$ with 95\% CI $[2.1 \times 10^{-21},\ 1.4 \times 10^{-13}]$, and that \textbf{20.6\% of the 10,000 LHS samples exceed the $10^{-15}$ threshold} under plausible joint-tail combinations of the input parameters. All simulations were executed in Python (SciPy, NumPy, Matplotlib).
...
To further maximize remediation efficiency, we \textit{hypothesize} a NemA catalytic variant (NemA$^{*2+}$) based on structural analogy to characterized OYE-family engineering. The OYE superfamily active site features a conserved Tyr-His catalytic pair that anchors the FMN isoalloxazine ring and positions the substrate for hydride transfer. A second shell of hydrophobic residues (Trp, Phe, Ile, Tyr, Leu, Met) flanks the substrate-binding pocket and governs the geometry of substrate approach, directly influencing $K_m$. Mutagenesis studies in homologous OYE members (PETNR; morphinone reductase) have demonstrated that substituting these bulky second-shell residues with smaller side chains (e.g., Trp$\rightarrow$Ala, Phe$\rightarrow$Gly) is consistent with an expanded substrate-binding pocket and reduced steric hindrance for non-canonical electron acceptors like chromate \cite{mowafy2010}.

\textbf{Note on parameter provenance and structural evaluation role.} The specific NemA$^{*2+}$ kinetic parameters used in the comparative systems model ($K_m^* = 16\ \mu$M, $k_{cat}^* = 37.2\ \text{h}^{-1}$) are \textbf{scenario-analysis assumptions} extrapolated from the OYE-family active-site mutagenesis precedent \cite{mowafy2010}. They are \textit{not} the output of the structural evaluation pipeline described in this paper; the structural pipeline is a \textit{consistency check} on the engineering hypothesis motivating NemA$^{*2+}$, not a kinetic prediction.

The structural evaluation pipeline (detailed in \S\ref{sec:pillar2b} and \S\ref{sec:structresults}) uses the 2.30 \AA\ X-ray crystal structure of NemA (PDB 8BPQ, UniProt P77258) to: (a) identify the FMN binding pocket and rank candidate second-shell mutations by a dimensionless contact-disruption score (\S\ref{sec:pillar2b}), and (b) evaluate CrO$_4^{2-}$ positioning relative to FMN via a geometric-placement analysis (\S\ref{sec:pillar2b}). The projected kinetic improvement, were the hypothesized mutant to be realized, would be a 3-fold reduction in $K_m$ and a 2-fold increase in $k_{cat}$ (in the operational units used by the circuit model), yielding a projected 6-fold improvement in $k_{cat}/K_m$ catalytic efficiency. The structural evaluation results (\S\ref{sec:structresults}) are reported as geometric observables only (Cr-FMN distances, H-bond donor counts, steric clash counts); they are not interpreted as evidence of catalytic improvement.
...
\subsection{Pillar 2b: Structural Evaluation of NemA$^{*2+}$ (\texttt{structural\_pipeline/})}
\label{sec:pillar2b}
The structural evaluation pipeline uses the 2.30 \AA\ X-ray crystal structure of \textit{E. coli} NemA (PDB 8BPQ, UniProt P77258 \cite{uniprotP77258}) to evaluate whether the proposed second-shell pocket-enlargement mutations (Mowafy \cite{mowafy2010} precedent) are \textit{structurally consistent} with reduced steric hindrance in the FMN pocket. The pipeline is a \textbf{structural consistency check}, not a kinetic prediction: it does not compute catalytic constants, does not perform molecular dynamics, and does not simulate the dynamic NADH-bound holoenzyme. Where the geometric observables are reported, they are reported as geometric facts about the optimized placement, not as evidence of catalytic improvement.

\textbf{Source structure and provenance.} PDB 8BPQ is the 2.30 \AA\ X-ray structure of E. coli NemA (UniProt P77258, 365 residues) with bound FMN cofactor, deposited in the PDB in 2023. The full machine-readable provenance record (RCSB source, fetch date, chain A residue count, cofactor inventory, and the Vina-fallback rationale) is in \texttt{results/structure\_provenance.json}.

\textbf{Step 1: Active-site identification and pocket analysis.} The FMN cofactor is located in chain A; the binding pocket is defined as all residues with any heavy atom within 5.0 \AA\ of any FMN heavy atom. First-shell residues are those within 3.5 \AA\ of FMN (direct contacts). Second-shell candidates are aromatic/hydrophobic residues (Trp, Phe, Tyr, Ile, Leu, Met) within 3.5--5.0 \AA\ of FMN that are not direct contacts. The candidate set is the engineering target for the Mowafy second-shell pocket-enlargement hypothesis. \texttt{analyze\_active\_site.py} performs this analysis; output is \texttt{results/nema\_active\_site.json} and \texttt{results/nema\_active\_site.png}.

\textbf{Step 2: Dimensionless contact-disruption score.} For each second-shell candidate residue $X$, a \textbf{contact-disruption score} is computed as the fraction of heavy-atom contacts lost when $X$ is in-silico truncated to alanine:
\begin{equation}
\mathrm{score}_X = \frac{C_X^{\mathrm{WT}} - C_X^{\mathrm{Ala}}}{C_X^{\mathrm{WT}}}
\end{equation}
where $C_X^{\mathrm{WT}}$ is the count of protein heavy atoms within 5.0 \AA\ of any $X$ heavy atom (excluding $X$'s own atoms), and $C_X^{\mathrm{Ala}}$ is the same count with $X$ trimmed to its alanine-like form (backbone N, CA, C, O + CB). The score is \textbf{dimensionless} (0--1), \textbf{not} a kcal/mol $\Delta\Delta G$ calculation. Score $< 0.3$ indicates a residue tolerant of $\rightarrow$ Ala substitution; $> 0.6$ indicates high structural cost. Methodological basis: contact-cutoff analysis as in Kannan \& Vishveshwara (J Mol Biol 292(2):441--464, 1999 \cite{kannan1999}) and the Rosetta community hotspot-residue screening. \texttt{scan\_contact\_disruption.py} performs this analysis; output is \texttt{results/nema\_contact\_disruption.csv} and \texttt{results/nema\_contact\_disruption.png}.

\textbf{Step 3: Pre-docking literature sanity check.} Before any substrate positioning analysis, \texttt{literature\_check.py} queries PubMed (NCBI E-utilities) to verify two preconditions for the structural evaluation: (Q1) whether docking chromate/Cr(VI) into an FMN-containing flavoprotein active site has published precedent, and (Q2) whether the mechanism of Cr(VI) reduction by NemA / OYE-family flavoproteins is reported as direct FMN hydride transfer or as indirect (e.g., ROS-mediated). The decision rule outputs either \texttt{structural\_compatibility} (literature supports the docking approach, Vina scores may be reported in the main text) or \texttt{exploratory\_positioning} (literature does not strongly support the approach, Vina scores go to supplementary). The 2026-07-01 run of this check returned \texttt{exploratory\_positioning} (0 docking-precedent papers in Q1; 0 direct-FMN-hydride papers in Q2; 2 ROS/Fenton-like mechanism papers in Q2). Output: \texttt{results/literature\_check.md} and \texttt{results/literature\_check.json}.

\textbf{Step 4: Geometric placement of CrO$_4^{2-}$.} AutoDock Vina 1.2.5 was attempted but rejected by Vina with \texttt{Atom type Cr is not a valid AutoDock type} (Vina's atom type set is C, A, N, O, P, S, H, F, I, NA, OA, SA, HD, plus a few metals: Mg, Mn, Zn, Ca, Fe, Cl, Br; Cr is not in this list). The fallback is \texttt{scipy.optimize.minimize}-based rigid-body geometric placement of CrO$_4^{2-}$ (Cr-O bond = 1.62 \AA, tetrahedral) with multi-start initialization ($N = 32$, seed = 42). The objective penalizes steric clashes (heavy-atom contacts $< 2.0$ \AA) and rewards H-bond-donor-like contacts (polar protein atoms within 3.5 \AA\ of any chromate O, with a Gaussian bonus centered at 2.8 \AA). The reported observables (Cr-FMN C4a distance, Cr-FMN N5 distance, H-bond donor count, steric clash count) are exact for the optimized placement; the placement itself is a heuristic. \texttt{geometric\_placement.py} performs this analysis; output is \texttt{results/nema\_docking\_results.csv}, \texttt{results/nema\_docking\_overlay.png}, and \texttt{results/structure\_provenance.json}.
...
\subsection{Pillar 4c: Global Uncertainty Quantification via Latin Hypercube Sampling (\texttt{biosafety\_lhs.py})}
\label{sec:pillar4c}
A one-factor-at-a-time sensitivity sweep does not capture joint-tail sensitivity: a parameter can be within its acceptable local range and still fail in combination with other uncertain parameters. Pillar 4c replaces the local sweep with a global Latin Hypercube Sampling (LHS) analysis over 8 input parameters drawn log-uniformly from literature-bounded ranges, with $N = 10{,}000$ samples (seed = 42, via \texttt{scipy.stats.qmc.LatinHypercube}).

The 8 sampled parameters and their literature-bounded log-uniform ranges (defined in \texttt{parameters.py}) are: per-bp per-generation mutation rate $\mu_{bp}$ \cite{drake1991,lee2012}, HGT intact-gene transfer probability for $\Delta thyA$ reversion, the same for $\Delta dapA$ \cite{dahlberg1998,muela1994,licht1999}, reactor volume $V$, maximum cell density $\rho_{\max}$, generations per day $g$, deployment window $T$ (the hypothesis states 30 days \cite{hypothesis}), and kill-switch target size $L_{\text{target}}$ \cite{chan2016}. The full bounds are reported in \texttt{parameters.py} and tabulated in the supplementary parameter table.

For each sample, the per-division combined failure probability is the product $p_{\text{combined}} = \mu_{bp} \cdot L_{\text{target}} \cdot p_{\text{HGT, thyA}} \cdot p_{\text{HGT, dapA}}$ (as in Pillar 4), the total cell count is $N = V \cdot \rho_{\max}$, the total generations is $G = g \cdot T$, and the 30-day escape probability is the exact Poisson form
\begin{equation}
P_{\text{escape}}^{30d} = 1 - e^{-N \cdot G \cdot p_{\text{combined}}}
\end{equation}
evaluated with the IEEE-754-safe \texttt{scipy.special.expm1} function (which computes $\exp(x) - 1$ with full precision even for $|x| \sim 10^{-17}$, replacing the manual Taylor-approximation branch used in \texttt{biosafety\_mutation\_model.py}). The full sample is written to \texttt{results/biosafety\_lhs.csv}; a 3-panel figure (CDF, log-binned histogram, Spearman rank-correlation heatmap) is written to \texttt{results/biosafety\_lhs.png}.

This global analysis provides three diagnostics the local sweep cannot: (a) the median $P_{\text{escape}}$ under the joint uncertainty; (b) a 95\% confidence interval; (c) the fraction of samples that exceed the $10^{-15}$ de-minimis threshold under plausible joint-tail combinations of the inputs.
...
To make the provenance of every numerical value in this manuscript traceable, Table~\ref{tab:provenance} lists each claim class, the evidence type, and the source file or citation. Claims are partitioned into: (i) \textbf{L}iterature-only --- values taken directly from prior published work; (ii) \textbf{Scenario assumptions} --- values chosen for the systems model that are not predicted by any structural pipeline in this study; (iii) \textbf{PREDICTED} --- outputs of the present study's computational models, with the responsible script.

\begin{table*}[!t]
\centering
\caption{Claim--evidence provenance for every numerical value in the manuscript. The split is intended to make the provenance of each value traceable to a single source file or citation. The NemA$^{*2+}$ kinetic row is explicitly tagged as a scenario assumption; the structural pipeline rows are tagged as geometric / structural observables, not kinetic predictions.}
\label{tab:provenance}
\small
\begin{tabular}{@{}p{4.5cm}p{3.5cm}p{8cm}@{}}
\toprule
\textbf{Claim} & \textbf{Evidence type} & \textbf{Source} \\
\midrule
Cr(VI) carcinogenicity and Bangladesh river concentrations & Literature & \cite{iarc1990}, \cite{rahman2021}, \cite{ahmed2022}, \cite{islam2020}, \cite{doem2022} \\
ChrB-sfGFP biosensor dose-response and LOD & Literature + this study's ODE & \cite{igem2013} + \texttt{circuit\_ode\_model.py} \\
Wild-type NemA kinetics on Cr(VI) ($K_m$, $k_{cat}$) & Literature & \cite{williams2003} \\
\textbf{NemA$^{*2+}$ kinetic parameters ($K_m^*$, $k_{cat}^*$)} & \textbf{Scenario assumption (NOT predicted by the structural pipeline)} & Assumed by OYE-family analogy \cite{mowafy2010}; see \S2.3.3 \\
NemA$^{*2+}$ candidate second-shell mutations (specific Trp/Phe/Ile/Tyr positions) & Structural pipeline (this study) & \texttt{structural\_pipeline/analyze\_active\_site.py} + \texttt{scan\_contact\_disruption.py} \\
Contact-disruption scores for second-shell candidates (dimensionless, 0--1) & Structural pipeline (this study) & \texttt{scan\_contact\_disruption.py}; methodological basis \cite{kannan1999} \\
CrO$_4^{2-}$ geometric positioning relative to FMN C4a & Structural pipeline (this study) & \texttt{geometric\_placement.py}; see \S3.5 and \S4.2 \\
Circuit burden 4.5\%, growth reduction 33\% & This study's ribosome-allocation model & \texttt{metabolic\_burden\_model.py} \\
96-hour Cr(VI) depletion dynamics & This study's ODE & \texttt{circuit\_ode\_model.py} \\
30-day $P_{\text{escape}}$ point estimate & Luria-Delbr\"uck Poisson + this study's parameters & \texttt{biosafety\_mutation\_model.py} \\
30-day $P_{\text{escape}}$ local sensitivity (Pillar 4b) & This study's one-factor sweep & \texttt{biosafety\_sensitivity.py} \\
30-day $P_{\text{escape}}$ global uncertainty (Pillar 4c) & This study's LHS (N = 10,000) & \texttt{biosafety\_lhs.py} \\
\bottomrule
\end{tabular}
\end{table*}
...
The structural evaluation pipeline (\S\ref{sec:pillar2b}) was executed end-to-end against PDB 8BPQ chain A. The complete machine-readable results are in \texttt{results/nema\_active\_site.json}, \texttt{results/nema\_contact\_disruption.csv}, \texttt{results/nema\_docking\_results.csv}, and \texttt{results/structure\_provenance.json}; the supporting figures are in \texttt{results/nema\_active\_site.png}, \texttt{results/nema\_contact\_disruption.png}, and \texttt{results/nema\_docking\_overlay.png}.

\textbf{Active-site composition.} The FMN binding pocket of NemA contains \textbf{26 residues within 5.0 \AA\ of FMN}, of which \textbf{13 are direct FMN contacts (within 3.5 \AA)} and \textbf{6 are second-shell aromatic/hydrophobic candidates} (Trp, Phe, Ile, Tyr with side chains in the 3.5--5.0 \AA\ shell). The 6 second-shell candidates ranked by minimum heavy-atom distance to FMN are: TRP 276 (3.59 \AA), PHE 323 (3.66 \AA), ILE 328 (3.97 \AA), PHE 351 (4.11 \AA), TYR 304 (4.50 \AA), and TRP 103 (4.95 \AA). Of these, ILE 328 is the most plausible target for the Mowafy second-shell pocket-enlargement hypothesis, as it is large (4-carbon branched side chain), peripheral (3.97 \AA), and not a direct FMN contact.

\textbf{Contact-disruption scores (dimensionless, 0--1, NOT kcal/mol).} The contact-disruption score for the 6 candidates is:
...
\textbf{Interpretation.} \textbf{No candidate scores in the "tolerant" band ($< 0.3$)}; all 6 fall in the intermediate band (0.3--0.6) or, in the case of TYR 304, the costly band ($> 0.6$). This is a \textbf{negative-to-mixed result} for the second-shell engineering hypothesis: the NemA FMN pocket does not contain a large hydrophobic residue whose substitution to Ala would be unambiguously tolerated by the contact-disruption criterion. \textbf{ILE 328} is the best candidate (lowest disruption at 0.369), but the score indicates that the mutation would involve measurable local structural rearrangement, not silent substitution. \textbf{TYR 304} is in the costly band and is the worst candidate. The structural pipeline does \textit{not} independently support the kinetic improvement hypothesized in the systems model; it is consistent with, but does not validate, the OYE active-site engineering precedent as applied to NemA's specific active-site geometry.

\textbf{Geometric placement of CrO$_4^{2-}$.} AutoDock Vina 1.2.5 was attempted but rejected with \texttt{Atom type Cr is not a valid AutoDock type} (Vina's empirical scoring function does not parameterize the Cr atom; see \texttt{results/structure\_provenance.json} for the full rationale). The fallback is a \texttt{scipy.optimize.minimize}-based rigid-body geometric placement of CrO$_4^{2-}$ (multi-start $N = 32$, seed = 42) for both wild-type and the in-silico ILE328ALA mutant. The optimized placement positions Cr at \textbf{$2.89 \pm 0.00$ \AA\ from FMN C4a} and \textbf{$3.44 \pm 0.00$ \AA\ from FMN N5}, with \textbf{7 H-bond donor contacts} (PRO 25, LEU 26, THR 27, SER 57, GLU 58, ALA 59, GLN 101) and \textbf{1 steric clash} (within 2.0 \AA).

\textbf{Honest observation.} The geometric placement analysis is \textbf{not sensitive} to the ILE328ALA mutation: WT and ILE328ALA produce identical observables. This is expected, because ILE 328 is 3.97 \AA\ from FMN (per the active-site analysis), which is far enough that removing 3 side-chain carbon atoms (CG1, CG2, CD1) does not affect the steric-clash or H-bond-donor profile near the FMN C4a. The geometric-placement analysis as formulated is sensitive to \textit{first-shell} mutations (residues within $\sim$3 \AA\ of FMN) but not to peripheral second-shell mutations. This is a methodological scope note, not a negative result on the engineering hypothesis; the contact-disruption score (above) is the appropriate tool for ranking peripheral mutations.

\textbf{Literature check (gate).} The pre-docking literature check (\texttt{results/literature\_check.md}) returned the decision \texttt{exploratory\_positioning}: 0 published papers were found that report docking chromate/Cr(VI) into an FMN-containing flavoprotein active site, 0 abstracts explicitly supported direct FMN hydride transfer, and 2 abstracts mention ROS/Fenton-like mechanisms. The structural analysis is therefore framed as an \textbf{exploratory structural-positioning} analysis, not a binding prediction. No Vina score is reported in the main text. The Cr--FMN distance and H-bond donor count are reported as geometric observables for the optimized placement only.
...
\subsubsection{Global Uncertainty Quantification: Latin Hypercube Sampling (Pillar 4c)}
\label{sec:lhs}
The local one-factor-at-a-time sensitivity sweep above is complemented by a global LHS analysis over 8 input parameters drawn log-uniformly from literature-bounded ranges ($N = 10{,}000$ samples, seed = 42, via \texttt{scipy.stats.qmc.LatinHypercube}; see \S\ref{sec:pillar4c} for the method).

\begin{figure*}[!t]
    \centering
    \includegraphics[width=0.8\textwidth]{simulations/results/biosafety_lhs.png}
    \caption{Global LHS uncertainty quantification of the 30-day $P_{\text{escape}}$ over 10,000 samples. (a) Cumulative distribution of $P_{\text{escape}}$ on a log-x axis, with the $10^{-15}$ threshold (orange dotted) and median (green dashed). (b) Histogram of $\log_{10}(P_{\text{escape}})$ over all samples. (c) Spearman rank correlation between each log-input and $\log_{10}(P_{\text{escape}})$; red = positive, blue = negative. Generated by \texttt{biosafety\_lhs.py}.}
    \label{fig:lhs}
\end{figure*}

\textbf{Headline LHS findings (N = 10,000 samples):}
\begin{itemize}
    \item \textbf{Median $P_{\text{escape}}$:} $1.71 \times 10^{-17}$ (within an order of magnitude of the point-estimate baseline of $6.00 \times 10^{-17}$, as expected for a log-uniform LHS centered on the baseline).
    \item \textbf{95\% confidence interval:} $[2.14 \times 10^{-21},\ 1.41 \times 10^{-13}]$ --- spans $\sim$8 orders of magnitude.
    \item \textbf{Fraction above the $10^{-15}$ de-minimis threshold:} \textbf{20.6\%}.
    \item \textbf{Top-3 Spearman rank correlations} (input $\to$ $P_{\text{escape}}$): HGT intact-gene transfer probability for $\Delta thyA$ reversion ($\rho = +0.57$), same for $\Delta dapA$ ($\rho = +0.55$), reactor volume $V$ ($\rho = +0.41$).
\end{itemize}

\textbf{Interpretation.} The global LHS analysis reveals that the headline $P_{\text{escape}} < 10^{-15}$ containment claim is \textbf{not robust to joint-tail combinations of the input parameters}, even though it is robust to local 10$\times$ errors in any single parameter (Pillar 4b). Under plausible combinations of literature-supported input values, \textbf{roughly 1 in 5 LHS samples exceeds the $10^{-15}$ threshold}. This is the most important quantitative finding of the present study: the de-minimis containment claim is a model output at the assumed input parameters, not a deep safety margin. The dominant drivers are the HGT intact-gene transfer probabilities (Spearman $\rho \approx 0.55$), for which the published literature provides bracketing ranges (Dahlberg 1998 reports 2.3$\times$10$^{-6}$ to 2.2$\times$10$^{-4}$ transconjugants per recipient in marine bacterial communities; the LHS bracketing range 10$^{-14}$ to 10$^{-10}$ per cell-division is conservative and acknowledged in \texttt{parameters.py}). The reactor volume and the two mutation-related parameters contribute less.

\textbf{Recommendation.} Until a direct measurement of the deployed per-bp mutation rate and the freshwater HGT intact-gene transfer probability is available, the 30-day containment claim should be read as the central estimate of an order-of-magnitude spread, not as a hard floor. A future wet-lab Luria-Delbrück fluctuation test on the deployed strain (Pillar 4b follow-up) and a freshwater HGT assay would convert the LHS from a sensitivity analysis into a validated probability statement.
...
(2) \textbf{NemA$^{*2+}$ kinetic parameters are scenario assumptions; the structural pipeline is a consistency check, not a kinetic prediction.} The kinetic parameters used for the NemA$^{*2+}$ variant ($K_m^* = 16\ \mu$M, $k_{cat}^* = 37.2\ \text{h}^{-1}$) are \textbf{scenario-analysis assumptions} extrapolated by analogy to active-site pocket mutagenesis precedent in the OYE family \cite{mowafy2010}. They are \textit{not} derived from the structural evaluation pipeline (\S\ref{sec:pillar2b}, \S\ref{sec:structresults}). The structural pipeline uses a dimensionless contact-disruption score and a geometric placement of CrO$_4^{2-}$ near the FMN pocket to evaluate whether the proposed second-shell mutations are \textit{structurally consistent} with reduced steric hindrance, but it does not predict or replace the kinetic values. The relationship between the structural evaluation and the systems model is one-way: the systems model uses scenario-analysis kinetic parameters, and the structural evaluation can support, qualify, or contradict the engineering hypothesis behind those parameters. The two are not coupled numerically. The 4-fold treatment-time reduction reported in Section~\ref{sec:structresults} is therefore a \textit{what-if} analysis of an enzyme variant that, according to the structural pipeline, is (or is not) a plausible engineering target --- not a prediction of how a real engineered NemA would behave.
...
(5) \textbf{Containment claim is not robust to joint-tail uncertainty.} The local Pillar 4b sensitivity sweep showed the headline $P_{\text{escape}} < 10^{-15}$ claim is robust to 10$\times$ errors in individual parameters, but the global Pillar 4c Latin Hypercube analysis (\S\ref{sec:lhs}) shows that \textbf{20.6\% of 10,000 LHS samples exceed the $10^{-15}$ threshold} under plausible joint-tail combinations of the 8 input parameters. The dominant drivers are the HGT intact-gene transfer probabilities ($\rho \approx 0.55$ for both $\Delta$thyA and $\Delta$dapA reversion), for which the published literature provides bracketing ranges (Dahlberg 1998 reports 2.3$\times$10$^{-6}$ to 2.2$\times$10$^{-4}$ transconjugants per recipient in marine bacterial communities; see \texttt{parameters.py} for the bracketing range used in the LHS). The literature on spontaneous \textit{E. coli} mutation rate is itself only constrained to within roughly one order of magnitude across studies, and the specific value for our chassis under our (closed-pond, burdened) growth conditions is not directly measured. The $10^{-15}$ risk threshold should be read as a model output at the assumed input parameters, not a deep safety margin.
...
(7) \textbf{Structural evaluation scope.} The structural pipeline (\S\ref{sec:pillar2b}) uses (a) a dimensionless contact-disruption score, not a kcal/mol $\Delta\Delta G$ calculation; (b) a geometric placement of CrO$_4^{2-}$ via \texttt{scipy.optimize.minimize}, not a free-docking calculation (AutoDock Vina 1.2.5 was attempted but rejected with \texttt{Atom type Cr is not a valid AutoDock type} --- Vina's empirical scoring function does not parameterize Cr; see \texttt{results/structure\_provenance.json} for the full rationale); and (c) the static crystal structure (PDB 8BPQ chain A), not the dynamic NADH-bound holoenzyme. The pre-docking literature check (\S\ref{sec:pillar2b}, \texttt{results/literature\_check.md}) returned the decision \texttt{exploratory\_positioning}: 0 published papers were found that report docking chromate/Cr(VI) into an FMN-containing flavoprotein active site, and 2 abstracts mention ROS/Fenton-like mechanisms for Cr(VI) reduction. Under that decision, the structural analysis is framed as an \textit{exploratory structural-positioning} analysis, not a binding prediction; no Vina score is reported in the main text, and the geometric observables (Cr--FMN distance, H-bond donor counts, steric clash counts) are reported as facts about the optimized placement only. The contact-disruption scores for the 6 second-shell candidates place all 6 in the intermediate band (0.3--0.6) or, in one case, the costly band ($> 0.6$); the structural pipeline does not independently support the kinetic improvement hypothesized in the systems model.

(8) \textbf{Mechanism of Cr(VI) reduction not established.} The pre-docking literature check did not find published evidence that Cr(VI) reduction by NemA-class flavoprotein oxidoreductases proceeds via direct FMN hydride transfer. If the actual mechanism is flavin-mediated ROS generation or a Fenton-like pathway, the geometric positioning of CrO$_4^{2-}$ relative to FMN C4a is not the catalytically relevant step, and the geometric observables reported in \S\ref{sec:structresults} are not informative about the catalytic competence of the proposed NemA$^{*2+}$ variant. A direct experimental measurement of the Cr(VI) reduction mechanism in NemA (e.g., anaerobic stopped-flow kinetics with isotope-labeled Cr(VI) and FMN) is the single experiment that would resolve this.
...
\begin{enumerate}
    ...
    \item A structural evaluation pipeline applied to the 2.30 \AA\ X-ray crystal structure of NemA (PDB 8BPQ) that uses a dimensionless contact-disruption score to rank 6 candidate second-shell mutations (none in the "tolerant" band; best candidate ILE 328 in the intermediate band) and a geometric placement of CrO$_4^{2-}$ (Cr-FMN C4a distance 2.89 \AA, 7 H-bond donor contacts, 1 steric clash) to evaluate substrate positioning. \textbf{The structural evaluation is a consistency check on the engineering hypothesis motivating NemA$^{*2+}$, not a kinetic prediction}; the kinetic parameters of NemA$^{*2+}$ remain scenario-analysis assumptions extrapolated from OYE-family active-site mutagenesis precedent \cite{mowafy2010}.
    ...
    \item A Luria-Delbr\"uck evolutionary biosafety analysis that, under the assumed input parameters, places the 30-day $P_{\text{escape}}$ of a viable escapee in a 1,000-liter closed system at $6 \times 10^{-17}$, below the $10^{-15}$ acceptable-risk threshold.
    \item A Pillar 4b local sensitivity analysis demonstrating that the headline $P_{\text{escape}} < 10^{-15}$ claim is robust to a 10$\times$ error in any single input parameter.
    \item A Pillar 4c \textbf{global Latin Hypercube Sampling analysis} ($N = 10{,}000$) over 8 input parameters drawn from literature-bounded ranges, finding a median $P_{\text{escape}} = 1.7 \times 10^{-17}$ (95\% CI $[2.1 \times 10^{-21}, 1.4 \times 10^{-13}]$) and a non-trivial \textbf{20.6\% of samples exceeding the $10^{-15}$ de-minimis threshold} under plausible joint-tail combinations. The dominant drivers are the HGT intact-gene transfer probabilities ($\rho \approx 0.55$).
\end{enumerate}

\textbf{One-paragraph bridge.} The relationship between the structural evaluation and the systems model is one-way: the systems model uses scenario-analysis kinetic parameters, and the structural evaluation can support, qualify, or contradict the engineering hypothesis behind those parameters. The two are not coupled numerically. The systems-model results for NemA$^{*2+}$ should therefore be read as a \textit{what-if} analysis of an enzyme variant that, according to the structural pipeline, is (or is not) a plausible engineering target --- not a prediction of how a real engineered NemA would behave. The contact-disruption score and the geometric placement are both screening tools, not kinetic predictors; the structural pipeline does not validate, derive, or predict catalytic constants.

This computational framework does not merely describe what the system should do --- it provides a transparent mathematical foundation to evaluate both the design's expected performance and the sensitivity of its safety claim to its key input assumptions, and to identify the specific experiments (mutation-rate measurement under deployment conditions; HGT intact-gene transfer assay in freshwater; direct measurement of the Cr(VI) reduction mechanism in NemA; BSL-1 chassis co-transformation) that would convert it from a design study into a validated one. All such experimental validation is explicitly out of scope for this manuscript.
...
\bibitem{dahlberg1998} Dahlberg C, Bergstr\"om M, Hermansson M. In situ detection of high levels of horizontal plasmid transfer in marine bacterial communities. \textit{Appl Environ Microbiol.} 64(7):2670--2675 (1998).
\bibitem{muela1994} Muela A, Pocino M, Arana I, Justo JI, Iriberri J, Barcina I. Effect of growth phase and parental cell survival in river water on plasmid transfer between \textit{Escherichia coli} strains. \textit{Appl Environ Microbiol.} 60(12):4273--4278 (1994).
\bibitem{licht1999} Licht TR. Plasmid transfer in the animal intestine and other dynamic bacterial populations: the role of community structure and environment. \textit{Microbiology (Reading).} 145(Pt 9):2615--2622 (1999).
\bibitem{kannan1999} Kannan N, Vishveshwara S. Identification of side-chain clusters in protein structures by a graph spectral method. \textit{J Mol Biol.} 292(2):441--464 (1999).
\bibitem{uniprotP77258} UniProt Consortium. \textit{UniProtKB P77258 (NEMA\_ECOLI)} --- N-ethylmaleimide reductase. \url{https://www.uniprot.org/uniprotkb/P77258}. Accessed 2026-07-01.
\bibitem{hypothesis} The 30-day closed-system deployment window is a stated central hypothesis of this work (Sec.~1.4, ``Central hypothesis''). It is not a literature-sourced parameter.
\end{thebibliography}
